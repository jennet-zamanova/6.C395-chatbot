...
class Chatbot:
    ...
    task = """
            ...
            IMPORTANT: once you have enough info, DO NOT search for facilities yourself, DO NOT give facility options yourself.

            You will be provided with SAMHSA’s directory contains thousands of treatment facilities nationwide, each with many attributes: treatment
    ...
    def __init__(self):
        ...
        self.model_id = MY_MODEL if MY_MODEL else BASE_MODEL # define MY_MODEL in config.py if you create a new model in the HuggingFace Hub
    ...
    def get_search_criteria(self) -> SearchCriteria:
        ...
        if not self.memory or len(self.memory) < 2:
            return {}
        extract_instruction = """From the conversation below, extract the user's preferences for finding a treatment facility. 
        Return a single JSON object only, no other text. 
        Use these exact keys when present: location_city, location_state, location_zip, 
        treatment_type (one of: inpatient, outpatient, residential, telehealth), 
        payment_options (list), 
        special_populations (list), 
        therapies (list), 
        languages (list), 
        substances (list). 
        Use empty list [] or omit the key if unknown. Examples: "Boston" -> location_city, "MA" or "Massachusetts" -> location_state."""
        conv_text = "\n".join(
        ...
        try:
            explanation_client = InferenceClient(model=self.model_id, token=HF_TOKEN)
            output = explanation_client.chat.completions.create(
                model=self.model_id,
                messages=messages,
                max_tokens=512,
            )
    ...
    def explain_facility_options(self, options: list[FacilityOption]) -> str:
        ...
        prompt = f"""Based on the user's conversation and these facility results, explain each option clearly and briefly: 
        name, location if present, key features (treatment type, payment, special populations, therapies, languages),
         and how it might fit their situation. Be warm and helpful.\n\nFacility options:\n{options_text}"""
